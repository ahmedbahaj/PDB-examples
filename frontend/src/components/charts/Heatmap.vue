...
const updateChart = () => {
  ...
  const filteredData = dataStore.interactions.filter(d => {
    if (dataStore.selectedInteractionTypes.size === 0) return false
    const typesString = d.typesArray.join('; ')
    return matchesSelectedTypes(typesString, dataStore.selectedInteractionTypes, INTERACTION_TYPES)
  })
  ...
  if (filteredData.length === 0) {
    ...
    chartContainer.value.innerHTML = '<div style="text-align: center; padding: 100px 20px; color: #6e6e73; font-size: 19px;">No interactions found. Try selecting different interaction types.</div>'
    return
  }
  ...
  chart = Highcharts.chart(chartContainer.value, {
    ...
    subtitle: {
      text: 'Chain A (X-axis) ↔ Chain B (Y-axis) | Color intensity = Consistency',
      style: {
        fontSize: '17px',
        color: '#6e6e73'
      }
